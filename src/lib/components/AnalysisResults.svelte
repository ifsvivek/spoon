<script>
	...
	function parseMarkdownContent(content) {
		try {
			...
			parsedContent.description = descriptionMatch?.[1]?.trim() || 'Document Analysis';
			...
			// Extract use cases
			const useCasesSection = content.match(/## Use Cases\n([\s\S]+?)(?=\n##|$)/);
			if (useCasesSection) {
			...
			const techSection = content.match(/## 🛠️ Technology Stack\n([\s\S]+?)(?=\n##|$)/);
			if (techSection) {
			...
			const installSection = content.match(/## 🚀 Getting Started\n([\s\S]+?)(?=\n##|$)/);
			if (installSection) {
...
<div class="mt-8 overflow-hidden rounded-lg bg-white shadow">
	{#if data.type === 'github'}
		...
		<div class="grid grid-cols-1 gap-6 px-6 py-5 md:grid-cols-2">
			<div>
				<h3 class="mb-3 text-lg font-medium text-gray-900">Project Summary</h3>
			...
			<div>
				<h3 class="mb-3 text-lg font-medium text-gray-900">Key Features</h3>
			...
			<div>
				<h3 class="mb-3 text-lg font-medium text-gray-900">Use Cases</h3>
				<ul class="list-disc space-y-1 pl-5 text-gray-700">
			...
			<div>
				<h3 class="mb-3 text-lg font-medium text-gray-900">Project Stats</h3>
	...
	{:else if data.type === 'document'}
		...
		<div class="border-b border-gray-200 px-6 py-5">
			...
			{/if}
			<div class="prose mt-3 max-w-none text-gray-600">
		...
		<div class="grid grid-cols-1 gap-6 px-6 py-5 md:grid-cols-2">
			...
			<div>
				<div class="mb-6">
					<h3 class="mb-3 text-xl font-semibold text-gray-900">Project Summary</h3>
				...
				{#if parsedContent.installation}
					<div class="mb-6">
						<h3 class="mb-3 text-xl font-semibold text-gray-900">Getting Started</h3>
						<div class="prose max-w-none text-gray-700">
			...
			<div>
				{#if parsedContent.features.length > 0}
					<div class="mb-6">
						<h3 class="mb-3 text-xl font-semibold text-gray-900">Key Features</h3>
						<ul class="space-y-2">
				...
				{#if parsedContent.useCases.length > 0}
					<div class="mb-6">
						<h3 class="mb-3 text-xl font-semibold text-gray-900">Use Cases</h3>
						<ul class="space-y-2">
				...
				{#if parsedContent.technologies.length > 0}
					<div class="mb-6">
						<h3 class="mb-3 text-xl font-semibold text-gray-900">Technology Stack</h3>
						<ul class="space-y-2">
		...
		<div class="border-t border-gray-200 bg-gray-50 px-6 py-5">
			<h3 class="mb-4 text-xl font-semibold text-gray-900">Document Statistics</h3>
			<div class="grid grid-cols-2 gap-4 md:grid-cols-4">
				<div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
					<p class="text-sm font-medium text-gray-500">Words</p>
					<p class="text-xl font-bold text-gray-800">
						{parsedContent.stats.words?.toLocaleString() || 'N/A'}
					</p>
				</div>
				<div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
					<p class="text-sm font-medium text-gray-500">Lines</p>
					<p class="text-xl font-bold text-gray-800">
						{parsedContent.stats.lines?.toLocaleString() || 'N/A'}
					</p>
				</div>
				<div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
					<p class="text-sm font-medium text-gray-500">File Size</p>
					<p class="text-xl font-bold text-gray-800">
						{formatFileSize(parsedContent.stats.size)}
					</p>
				</div>
				<div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
					<p class="text-sm font-medium text-gray-500">Sections</p>
					<p class="text-xl font-bold text-gray-800">
						{parsedContent.stats.sections || 'N/A'}
					</p>
				</div>
			</div>
		</div>
